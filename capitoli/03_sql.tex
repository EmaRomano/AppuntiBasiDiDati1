...
\begin{bestpractice}[Design Pattern: Il Record Fittizio (Dummy Record)]
...
\begin{itemize}
    ...
    \item \textbf{Sinergia con le politiche referenziali:} Questo pattern rende implementabile in modo logico la direttiva \texttt{ON DELETE SET DEFAULT}. Se un'entità padre viene eliminata, i record figli ad essa associati non assumono il valore \texttt{NULL}, ma vengono automaticamente riassegnati al record "Sconosciuto" (valore di default), preservando la continuità strutturale dei dati.
\end{itemize}
\end{bestpractice}
