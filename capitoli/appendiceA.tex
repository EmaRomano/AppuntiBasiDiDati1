...
\chapter[Ottimizzazione, Prestazioni, Scalabilità]{Cenni su Ottimizzazione, Prestazioni\\e Scalabilità dello Storage}
...
Poiché l'indirizzo viene calcolato matematicamente e non cercato fisicamente tramite salti tra le pagine, l'indice Hash evita la fase di navigazione a strati del B-Tree. Il costo computazionale per recuperare un record tramite un indice Hash è \textbf{Tempo Costante ($O(1)$)}. Che il database contenga dieci righe o dieci miliardi di righe, il tempo medio per decodificare la chiave e trovare il record esatto rimane identico e pressoché istantaneo.
...
In questa architettura (che è il comportamento \textbf{di default in PostgreSQL}), la tabella vera e propria è un \textit{Heap File}: un file non ordinato in cui le nuove righe vengono semplicemente accodate nel primo blocco di memoria libero disponibile.
L'indice (es. sulla Chiave Primaria) viene costruito come una struttura B-Tree separata. Le foglie di questo albero non contengono i dati della riga, ma solo un puntatore (chiamato \textit{Tuple ID} o \textit{TID}) che indica l'esatto offset nel file di Heap. 
Gli indici che "puntano" a un file separato sono definiti \textbf{Indici Secondari} (\textit{Non-Clustered Indexes}).
...
\begin{observation}[Il costo del Pointer Lookup]
Perché questa distinzione è vitale per le prestazioni? Se un'interrogazione utilizza un indice secondario (come fa nativamente PostgreSQL) per trovare un record, il motore deve prima attraversare il B-Tree per trovare il puntatore, e \textbf{successivamente} eseguire un ulteriore salto sul disco verso l'Heap File per estrarre le colonne richieste. Questo "salto extra" (\textit{Pointer Lookup}) su dischi meccanici può risultare un collo di bottiglia severo se la query estrae decine di migliaia di righe sparse in blocchi diversi del disco.
\end{observation}
