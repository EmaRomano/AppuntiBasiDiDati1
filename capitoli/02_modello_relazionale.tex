...
Nel modello relazionale, questo legame si traduce mantenendo due schemi di relazione separati (due tabelle) e introducendo una \textbf{Chiave Esterna (FK)} in uno dei due schemi che punta alla Chiave Primaria (PK) dell'altro.

\subsubsection{Il vincolo UNIQUE sulla Chiave Esterna}
...
Per garantire la biunivocità dell'associazione, è obbligatorio imporre un \textbf{vincolo di univocità} (in SQL, il vincolo \texttt{UNIQUE}) sugli attributi che compongono la chiave esterna.
...
\subsubsection{Dove posizionare la Chiave Esterna?}
La scelta di quale delle due tabelle debba ospitare la FK dipende dalle \textbf{cardinalità minime} (l'opzionalità o obbligatorietà dell'associazione):
...
\begin{itemize}
    \item $\texttt{PROFESSORE}(\underline{\text{matricola}}, \text{nome}, \text{cognome})$
    \item $\texttt{DIPARTIMENTO}(\underline{\text{codice}}, \text{nome}, \underline{\underline{\text{direttore}}})$
\end{itemize}

Vincoli su \texttt{DIPARTIMENTO.direttore}:
\begin{itemize}
    \item \textbf{Foreign Key:} punta a \texttt{PROFESSORE.matricola}
