...
Nella modellazione di un database relazionale, ogni riga di una tabella deve poter essere identificata in modo univoco (Chiave Primaria). In molti scenari reali, i dati "naturali" di un'entità non sono adatti a questo scopo: i nomi omonimi esistono, le email possono essere cambiate e persino i codici fiscali possono essere soggetti a ricalcolo. 

Come sappiamo, per garantire un'identificazione robusta e immutabile nel tempo, l'ingegneria del software ricorre quasi sempre alle \textbf{Chiavi Surrogate}: attributi artificiali, tipicamente un numero intero progressivo (es. 1, 2, 3...), privi di qualsiasi significato logico per l'utente, ma utilizzati dal motore relazionale per collegare le tabelle tra loro in modo sicuro.
