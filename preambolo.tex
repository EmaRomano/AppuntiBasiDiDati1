...
\usepackage{float}           % Per forzare il posizionamento di tabelle
...
% Definizione del dialetto PostgreSQL esteso
\lstdefinelanguage{PostgreSQL}[]{SQL}{
    morekeywords={
        TO, SHOW, SET, SERIAL, search_path, current_schema, TIMESTAMP, 
        VARCHAR, NUMERIC, DEFAULT, UNIQUE, GENERATED, ALWAYS, BIGINT,
        datname, pg_database, schema_name, schema_owner, 
        information_schema, schemata
    },
    keywordstyle=\color{blue}\bfseries
}

% Definizione del dialetto Bash esteso per il terminale Linux
\lstdefinelanguage{TerminalBash}[]{bash}{
    morekeywords={sudo, systemctl, start, enable, stop, status, clear, ls, cd, psql},
...
% --- MACRO PER SIMBOLI OUTER JOIN ---
\usepackage{amssymb} % Assicurati di avere questo pacchetto (probabilmente lo hai già)
